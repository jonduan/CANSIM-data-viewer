...
In general, the list of methods and packages is in no way meant to be exhaustive. Moreover, the code attached is not elaborated upon in tremendous detail. The assumption is that enough information is provided that it anyone unfamiliar with the specific packages (but not unfamiliar with R as a whole) can quickly grasp the concept and proceed to play around with the code to get a sense of how they can utilize these packages for real-world applications. All the packages and functions mentioned here have varying degrees of documentation and numerous examples online. In most cases, a simple Google search on the name of the package can instantly provide anyone with a great deal of useful information. 
...
Due to a tremendous amount of work by individuals across the globe, there are now multiple ways for R users to utilize various well-known JavaScript visualization libraries while having little to no knowledge of the JavaScript language itself. Packages such as `rCharts` and `dygraphs` take user-provided R code and translate it to JavaScript code in order to leverage the relevant charting libraries. The benefit of using these packages as opposed to `ggplot2` with `ggiraph` is that they offer a much higher degree of interactivity and user input. However, the downside is that each such package has its own syntax, hence necessitating some investment of time and effort on the part of the analyst before getting started. 
...
```
<br>
The above graph is very busy and unsightly, but the intention is to show the numerous options and the excellent performance of `dygraphs` for graphing large time series datasets. The package supports multiple timeseries, event-shading, pinch-to-zoom, horizontal scrolling etc. All of the options can be modified depending on the specific use case.  

In addition, it is once again worth pointing out that even though `dygraphs` has a fairly consistent and easy-to-follow syntax, it is nonetheless different than the other packages showcased thus far. 
